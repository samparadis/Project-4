...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        arguments = rest.map(lambda expression: scheme_eval(expression, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        return target
        # END PROBLEM 6
    # Target is the define part
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        formals, body = target.second, expressions.second
        lambda_pair = Pair(formals, body)
        lambda_function = do_lambda_form(lambda_pair, env)
        env.define(target.first, lambda_function)
        return target.first
        # END PROBLEM 10
    else:
...
def do_if_form(expressions, env):
    ...
    elif expressions.second.second != nil:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.second is not nil:
            return_value = scheme_eval(expressions.first, env)
        else:
            return_value = scheme_eval(expressions.first, env, True)
        if scheme_falsep(return_value):
            return False
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    formal, expression = nil, nil
    while bindings != nil:
        current_binding = bindings.first
        check_form(current_binding, 2, 2)
        if not scheme_symbolp(current_binding.first):
            raise SchemeError
        formal = Pair(current_binding.first, formal)
        expression = Pair(scheme_eval(current_binding.second.first, env), expression)
        bindings = bindings.second
    return env.make_child_frame(formal, expression)
    # END PROBLEM 15
